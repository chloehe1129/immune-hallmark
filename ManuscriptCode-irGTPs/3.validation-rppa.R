...
# GG-corrplot - Myeloid
a = Mclust(ssGSEA_M[4,], G=2, model="V")
b = Mclust(ssGSEA_M[6,], G=2, model="V")
c = Mclust(ssGSEA_M[3,], G=2, model="V")
d = Mclust(ssGSEA_M[7,], G=2, model="V")
e = Mclust(ssGSEA_M[8,], G=2, model="V")
adhesion = c("CD49d","CD29","CD62L",'CD62P') # ,'CD62L'
pvalue = matrix(1,4,10)
df = as.data.frame(cbind(a$classification,ssGSEA_M[4,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
df1 = aggregate(df,by=list(df$V1),FUN=mean)[,c(4:7)]
...
df = as.data.frame(cbind(b$classification,ssGSEA_M[6,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
df2 = aggregate(df,by=list(df$V1),FUN=mean)[,c(4:7)]
pvalue[,3]=c(t.test(df[which(df$V1==1),3],df[which(df$V1==2),3])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),4])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),5])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),6])$p.value)
df = as.data.frame(cbind(c$classification,ssGSEA_M[3,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
df3 = aggregate(df,by=list(df$V1),FUN=mean)[,c(4:7)]
pvalue[,5]=c(t.test(df[which(df$V1==1),3],df[which(df$V1==2),3])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),4])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),5])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),6])$p.value)
df = as.data.frame(cbind(d$classification,ssGSEA_M[7,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
df4 = aggregate(df,by=list(df$V1),FUN=mean)[,c(4:7)]
pvalue[,7]=c(t.test(df[which(df$V1==1),3],df[which(df$V1==2),3])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),4])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),5])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),6])$p.value)
df = as.data.frame(cbind(e$classification,ssGSEA_M[8,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
df5 = aggregate(df,by=list(df$V1),FUN=mean)[,c(4:7)]
pvalue[,9]=c(t.test(df[which(df$V1==1),3],df[which(df$V1==2),3])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),4])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),5])$p.value,
             t.test(df[which(df$V1==1),3],df[which(df$V1==2),6])$p.value)
df = as.matrix(cbind(t(df1),t(df2),t(df4),t(df5)))
df = apply(df,1,scale)
df[2,] = df[2,]+runif(4,1,2)
rownames(df) = c("Low","High","Low","High","Low","High","Low","High")
pdf("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/manuscript/protein_PBMC_validation_M1-4.pdf",
    width=12,height=4)
...
ann_col <- data.frame(factor(c(rep("Cell_Adhesion_1",2),rep("Cell_Adhesion_2",2),
                               rep("Eosinophil_chemotaxis",2),rep("Chemokine_activity",2)),
                             levels = rownames(ssGSEA_M)[c(4,6,3,7,9)]))
...
colAnn <- HeatmapAnnotation(#df = ann_col,
  ...
  foo = anno_block(gp = gpar(fill = c("lightblue2","lightcoral","lightpink","cornflowerblue","gold")),
                   labels = c("M_MP4: Cell Adhesion.1","M_MP6: Cell Adhesion.2",
                              "M_MP7: Eosinophil Chemotaxis","M_MP9: Chemokine Activity")),
...
Heatmap(t(df),name = "Expression" ,cluster_rows = F,cluster_columns = F,
        ...
        column_split=factor(c(rep("Cell_Adhesion_1",2),rep("Cell_Adhesion_2",2),
                              rep("Eosinophil_chemotaxis",2),rep("Chemokine_activity",2)),
                            levels = rownames(ssGSEA_M)[c(4,6,7,9)]),
)
dev.off()

adhesion = c("CD49d","CD29","CD62P","CD44") # ,'CD62L'
df = as.data.frame(cbind(a$classification,ssGSEA_M[2,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
...
df = as.data.frame(cbind(b$classification,ssGSEA_M[2,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
names(df) = c("Group","MP",adhesion)
df$Group = as.factor(df$Group)
dflong = melt(df, id.vars=c("Group","MP"))
p2=ggplot(dflong,aes(x=Group,y=value,fill=Group))+geom_violin()+stat_compare_means()+
  ...
  facet_wrap(~variable,scales="free")+theme(text = element_text(size = 15)) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)))
df = as.data.frame(cbind(c$classification,ssGSEA_M[2,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
names(df) = c("Group","MP",adhesion)
df$Group = as.factor(df$Group)
dflong = melt(df, id.vars=c("Group","MP"))
p3=ggplot(dflong,aes(x=Group,y=value,fill=Group))+geom_violin()+stat_compare_means()+
  ...
  facet_wrap(~variable,scales="free")+theme(text = element_text(size = 15)) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)))
df = as.data.frame(cbind(d$classification,ssGSEA_M[2,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
names(df) = c("Group","MP",adhesion)
df$Group = as.factor(df$Group)
dflong = melt(df, id.vars=c("Group","MP"))
p4=ggplot(dflong,aes(x=Group,y=value,fill=Group))+geom_violin()+stat_compare_means()+
  ...
  facet_wrap(~variable,scales="free")+theme(text = element_text(size = 15)) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)))
df = as.data.frame(cbind(e$classification,ssGSEA_M[2,],t(protein_exp[adhesion[which(adhesion%in%rownames(protein_exp))],])))
names(df) = c("Group","MP",adhesion)
df$Group = as.factor(df$Group)
dflong = melt(df, id.vars=c("Group","MP"))
...
pdf("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/manuscript/protein_PBMC_validation_M2-3.pdf",height=3,width=9)
...
pdf("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/manuscript/protein_PBMC_validation_L2-5.pdf",
    width=8,height=4)
col_fun = colorRamp2(c(-2,0,2), c("deepskyblue","white","hotpink3"))
ann_col <- data.frame(factor(c(rep("Cell Adhesion",2),rep("Cell Adhesion(CD52+)",2)),levels = c("Cell Adhesion","Cell Adhesion(CD52+)")))
colnames(ann_col) <- c("MP")
