...
files = list.files("/Volumes/she4/hallmark/data/spatial/intestine-scRNA/",pattern = "matrix.mtx",full.names = T)
barcode_files = list.files("/Volumes/she4/hallmark/data/spatial/intestine-scRNA/",pattern = "barcode",full.names = T)
feature_files = list.files("/Volumes/she4/hallmark/data/spatial/intestine-scRNA/",pattern = "genes",full.names = T)
mat_all = readMM(files[1])
colnames(mat_all) = paste0(toupper(strsplit(strsplit(files[1],"-")[[1]][3],"\\.")[[1]][1]),"@",read.table(barcode_files[1])$V1)
rownames(mat_all) = read.table(feature_files[1])$V2
for(i in 2:length(files)){
  mat = readMM(files[i])
  colnames(mat) = paste0(toupper(strsplit(strsplit(files[i],"-")[[1]][3],"\\.")[[1]][1]),"@",read.table(barcode_files[i])$V1)
  rownames(mat) = read.table(feature_files[i])$V2
  mat_all = cbind(mat_all,mat)
  print(i)
}
ic_scRNA_celllabels = read.csv("/Volumes/she4/hallmark/data/spatial/intestine-scRNA/CRC_GSE139555_CellMetainfo_table.tsv",sep="")
ic_scRNA_celllabels = ic_scRNA_celllabels[,c("Cell","Celltype.1")]
names(ic_scRNA_celllabels) = c("SpotID","CellType")
common_cells = intersect(ic_scRNA_celllabels$SpotID,colnames(mat_all))
ic_scRNA_celllabels = ic_scRNA_celllabels[!duplicated(ic_scRNA_celllabels$SpotID),]
ic_scRNA_celllabels = ic_scRNA_celllabels[which(ic_scRNA_celllabels$SpotID%in%common_cells),]
...
write.table(ic_scRNA_celllabels,"/Volumes/she4/hallmark/data/spatial/CytoSPACE_intestine/ic_scRNA_celllabels.txt",
            row.names = F,quote = F,sep="\t")
scrna_count = scrna_count[-which(apply(scrna_count[,-1],1,sum)==0),]
write.table(scrna_count,"/Volumes/she4/hallmark/data/spatial/CytoSPACE_intestine/ic_scRNA_GEP.txt",
            row.names = T,quote = F,sep="\t")

intestine = Load10X_Spatial(paste0(path_data,"spatial/intestine/"))
ic_STdata_coordinates = cbind(colnames(intestine),intestine@images$slice1@coordinates[,c("row","col")])
names(ic_STdata_coordinates)[1] = "SpotID"
write.table(ic_STdata_coordinates,"/Volumes/she4/hallmark/data/spatial/CytoSPACE_intestine/ic_STdata_coordinates.txt",
            row.names = F,quote = F,sep="\t")
ic_STdata_GEP = as.data.frame(cbind(rownames(intestine),as.matrix(intestine@assays$Spatial$counts)))
names(ic_STdata_GEP)[1] = "Gene"
write.table(ic_STdata_GEP,"/Volumes/she4/hallmark/data/spatial/CytoSPACE_intestine/ic_STdata_GEP.txt",row.names = F,quote = F,sep="\t")
...
brast_ST = Load10X_Spatial(paste0(path_data,"spatial/intestine/"))
...
TLS = read.csv("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/10X_pathologist/intestine/TLS.csv",
               row.names = 1)
TLS$TLS = ifelse(TLS$TLS=="TLS","TLS","Not TLS")
brast_ST = AddMetaData(brast_ST,metadata = TLS)
pdf("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/manuscript/Spatial_TLS_intestine.pdf",
    height=4,width=4)
VlnPlot(brast_ST,features = "L_TCR_Anchoring1",group.by = "TLS",pt.size=0,
        cols = c("hotpink","cornflowerblue"))+
  ggpubr::stat_compare_means(label="p.signif")+ylim(-2.5,4)
dev.off()

malignancy = read.delim(paste0(path_data,"spatial/intestine/IC_copykat_prediction.txt"))
rownames(malignancy) = malignancy$cell.names
brast_ST = AddMetaData(brast_ST,metadata = malignancy)

# pdf("~/desktop/intestine_Spatial_Signature_score_G1.pdf",height=10, width=10,onefile = T)
# for (i in 4:31){ 
...
pdf("~/desktop/MP_Spatial_Signature_score_G1_intestine.pdf",height=10, width=10,onefile = T)
for (i in 4:31){ 
...
srt = readRDS(paste0(path_data,"spatial/intestine-scRNA/intestine.rds"))
...
saveRDS(celltrek,paste0(path_result,"spatial/celltrek_intestine.rds"))
celltrek = readRDS(paste0(path_result,"spatial/celltrek_intestine.rds"))
celltrek = readRDS("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/celltrek/celltrek_intestine.rds")
...
CellTrek::celltrek_vis(celltrek@meta.data %>% dplyr::select(coord_x, coord_y, Celltype..major.lineage.:id_new),
...
celltrek1$Celltype..major.lineage. = factor(celltrek1$Celltype..major.lineage.,
                                  levels = c("B","Plasma","CD4Tconv","CD8T","Tprolif","DC","Mono/Macro","Mast",
...
saveRDS(celltrek1,paste0(path_result,"spatial/celltrek_intestine.rds"))
pdf("/Users/she4/Library/CloudStorage/OneDrive-InsideMDAnderson/ken/hallmark/manuscript/spatial_clustered_dotplot_intestine_Hallmark.pdf",
    height=7, width=9)
Clustered_DotPlot(celltrek1,features = names(gs_list)[40:89],k=3, # [88:115]
                  ...
                  colors_use_idents = c("khaki1","khaki4","plum2", "mediumpurple2","violetred",
                                                "cadetblue1","cornflowerblue","navy",
