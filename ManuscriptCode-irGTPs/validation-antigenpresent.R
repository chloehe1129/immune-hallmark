...
pvalue = fgsea(pathways = c(gs_list_L,gs_list_M), stats = ranks)

pvalue = fgsea(pathways = gs_list_L, stats = ranks)
p0=plotEnrichment(gs_list_M[[2]],ranks)+ labs(title="Antigen_Processing")
p0$layers[[5]]$aes_params$colour <- 'purple'
p0 = p0+labs(subtitle = paste0("NES: ",round(pvalue[which(pvalue$pathway == "Antigen_Processing"),6],3)," Adjusted p-value: ",
                                 round(pvalue[which(pvalue$pathway == "Antigen_Processing"),3],3)))+
...
gsea$significance = ifelse(gsea$p.adjust<0.05,"Significant","Non-Significant")
pdf(paste0(path_result,"validation/antigen_pTMB_all.pdf"),height=6,width = 9)
ggplot(gsea, aes(x = NES, y = Description)) +
  geom_point(aes(size = NES, color = significance)) +
